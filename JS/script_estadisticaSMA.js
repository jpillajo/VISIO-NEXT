...
function actualizarGrafico(id_provincia) {
    ...
    div.insertAdjacentHTML("afterbegin", '<canvas class="my-4 w-100 h-10" id="myChart"></canvas>'); //adding the canvas again
}
...
function recargarProvincias() {
    $.ajax({
        type: "POST",
        url: "provincias.php",
        data: "provincia=" + "SELECT tbl_provincia.id_provincia AS provincia_id, nombre_provincia, ROUND(cobertura_2g_3g::numeric*100, 2) AS cobertura_2g_3g, ROUND(cobertura_4g::numeric*100, 2) AS cobertura_4g FROM tbl_sma_porcentaje, tbl_provincia WHERE tbl_provincia.id_provincia = tbl_sma_porcentaje.id_provincia;",
        success: function(r) {
